...
export default function WhatsAppButton({ lang }) {
  ...
  const [render, setRender] = useState(false) // keeps the list mounted through its exit animation
  ...
  // Mount the list as soon as it opens; on close, keep it mounted for the
  // length of the exit animation, then unmount.
  useEffect(() => {
    if (open) { setRender(true); return }
    if (!render) return
    const tmr = setTimeout(() => setRender(false), 220)
    return () => clearTimeout(tmr)
  }, [open, render])
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes wa-list-in {
          0%   { opacity: 0; transform: translateY(16px) scale(0.92); }
          60%  { transform: translateY(-2px) scale(1.01); }
          100% { opacity: 1; transform: translateY(0) scale(1); }
        }
        @keyframes wa-list-out {
          0%   { opacity: 1; transform: translateY(0) scale(1); }
          100% { opacity: 0; transform: translateY(16px) scale(0.92); }
        }
        /* The contact list is kept mounted through its close so it can run
           an exit animation — a conditionally-rendered element unmounts
           instantly and can only ever animate in. A tiny state machine in
           the component (render + open) drives which animation plays. */
        .wa-list {
          display: flex; flex-direction: column; gap: 0.5rem;
          align-items: flex-end;
          transform-origin: bottom right;
          animation: wa-list-in 0.26s cubic-bezier(0.34,1.56,0.64,1) both;
        }
        .wa-list.closing {
          animation: wa-list-out 0.2s ease both;
          pointer-events: none;
        }
        /* Icon morph on the main button: chat and X are stacked and
           cross-rotate/fade so pressing it spins between the two states
           instead of hard-swapping the SVG. */
        .wa-icons { position: relative; width: 30px; height: 30px; }
        .wa-ic {
          position: absolute; inset: 0;
          display: flex; align-items: center; justify-content: center;
          transition: transform 0.32s cubic-bezier(0.4,0,0.2,1), opacity 0.22s ease;
        }
        .wa-ic-chat { opacity: 1; transform: rotate(0deg) scale(1); }
        .wa-ic-x    { opacity: 0; transform: rotate(-90deg) scale(0.5); }
        .wa-main-btn.open .wa-ic-chat { opacity: 0; transform: rotate(90deg) scale(0.5); }
        .wa-main-btn.open .wa-ic-x    { opacity: 1; transform: rotate(0deg) scale(1); }
        .wa-contact-card {
          display: flex;
          align-items: center;
          gap: 0.85rem;
      ...
      <div style={{
        ...
        {/* Contact list */}
        {render && (
          <div className={`wa-list${open ? '' : ' closing'}`}>
